...
function extractGlbUrl(data: Record<string, unknown>): string | undefined {
  return (
    ...
    ((data.outputs as Record<string, unknown> | undefined)?.glb as
      | string
      | undefined) ||
    ((data.model_mesh as Record<string, unknown> | undefined)?.url as
      | string
      | undefined)
  );
}
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (!glbUrl) {
      console.error(
        "[refine-mesh/status] No GLB URL in response. Keys: %s. Full data: %s",
    ...
    console.error(
      "[refine-mesh/status] UNHANDLED ERROR:",
      error instanceof Error
